...
export const updateOffer = async(req, res, next)=>{
    const {offerId, sender, recipient, application} = req.body
    if(!offerId) return next(customError(401, "offer Not Found"))
    try {
        const updateOffer = await jobOfferModel.findByIdAndUpdate(offerId, req.body)
        if(!updateOffer) return(next(customError(500, "unable to update")))
        // ---------Update employer and jobSeeker------
        const newNotification = new NotificationModel({
            sender,
            recipient,
            application,
            message: updateOffer.status
        })

        // ---------Create a notification and update---------
        const savedNotification = await newNotification.save()
        await jobSeekerReportModel.findByIdAndUpdate(recipient,{$push: {notifications: savedNotification._id}})
        await jobEmployerModel.findByIdAndUpdate(sender,{$push: {notifications: savedNotification._id}})
        res.status(200).json({offer: updateOffer, notification: savedNotification})
    } catch (error) {
        next(error)
    }
}
